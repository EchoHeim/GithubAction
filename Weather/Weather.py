...
class MonitorWeather:
    ...
    def printText(self, daily, date, minCode, maxCode, extraWord, suggestion):
        ...
        if minCode <= code_day <= maxCode or minCode <= code_night <= maxCode:
            ...
            wendu = (
                ...
                + " ~ "
                + json["results"][0]["daily"][daily]["high"]
            )
            ...
            result = (
                dayText + nightText + " ," + extraWord + wendu + fengdengji + suggestion
            )
            # print(result)
    ...
    def Url(self):
        ...
        print(sign)
        self.dingdingurl = (
            "https://oapi.dingtalk.com/robot/send?access_token=cad451762de13cc3c67275fe9e939ea3f58cca657a3cac94faedb220952df0a7"
            + "&timestamp="
            + timestamp
            + "&sign="
            + sign
    ...
    def send(self):
        ...
        curTime = (datetime.datetime.now() + datetime.timedelta(hours=8)).strftime(
            "%Y-%m-%d %H:%M:%S"
        )
        print(curTime)
        data = {
            "msgtype": "text",
            "text": {"content": f"{curTime}    上海" + "\n" + self.result},
            "at": {
                # "atMobiles": [
                #     "156xxxx8827",
                #     "189xxxx8325"
...
if __name__ == "__main__":
    ...
    suggestion = (
        " ,感冒: "
        + suggestionJson["results"][0]["suggestion"]["flu"]["brief"]
        + "  紫外线: "
        + suggestionJson["results"][0]["suggestion"]["uv"]["brief"]
    )
    # print(suggestion)
